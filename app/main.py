...
async def add_embed_headers(request, call_next):
    ...
    if shop:
        csp_value = f"frame-ancestors https://{shop} https://admin.shopify.com;"
    else:
        # Fallback: allow all for development (not recommended for production)
        csp_value = "frame-ancestors *;"
    
    response.headers["Content-Security-Policy"] = csp_value
    return response

# Include routers - home router must be included first to handle root path
